...
class AppGUI:
    ...
    def gen_widget(self):
        ...
        if self.all_data != {}:
            for param, data in self.all_data.items():  # <-- Nutze all_data
                ...
                tol_low.bind("<Return>", lambda e, p=param: self.on_input(p))
                ...
                tol_up.bind("<Return>", lambda e, p=param: self.on_input(p))
    ...
    def on_mean(self, p):
        ...
        win.geometry(f"250x132+{self.root.winfo_rootx()+self.root.winfo_width()-250}+{self.root.winfo_rooty()+50}")
